...
with open('data/tfl_stations_new.json', 'w') as out:
    out_json = {
...
with open('data/tfl_lines.json', 'r') as json_file:
    ...
    for line in all_lines:
        new_line = {
            ...
            # 'start_sid': line['properties']['start_sid'],
            # 'end_sid': line['properties']['end_sid']
        }
        new_lines = []
        for l in line['properties']['lines']:
            if l['name'] in lines:
                ...
                if line_name == "London Overground":
                    line_name = "Overground"
                new_lines.append(line_name)
        new_line['Lines'] = ', '.join(new_lines)
        if len(new_lines) > 0:
            new_line['Line'] = new_lines[0]
            new_connections.append(new_line)

with open('data/tfl_lines_new.json', 'w') as out:
    out_json = {
        "type": "FeatureCollection",
